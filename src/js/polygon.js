...
function onLoad() {
	...
	svg.setAttribute('id', 'polygon-bg');
	svg.setAttribute('width', window.innerWidth);
	svg.setAttribute('height', window.innerHeight);
	document.querySelector(parentElement).prepend(svg);
	...
	for (let i = 0; i < points.length; i++) {
		if (points[i].originX != unitWidth * (numPointsX - 1) && points[i].originY != unitHeight * (numPointsY - 1)) {
			...
			for (let n = 0; n < 2; n++) {
				...
				polygon.setAttribute('stroke', 'rgba(100,100,100,' + (Math.random() / 1) + ')');
...
function refresh() {
	randomize();
	for (let i = 0; i < document.querySelector(parentElement + ' svg').childNodes.length; i++) {
		let polygon = document.querySelector(parentElement + ' svg').childNodes[i];
...
function onResize() {
	let svg = document.querySelector('#polygon-bg');
	if (svg) svg.remove();
